...
void ConfigureEPWM1(void)
{
    ...
    // 设置EPWM模块的计数方式
    EPwm1Regs.TBCTL.bit.CTRMODE = TB_FREEZE;        // 设定为增减计数模式，产生对称的EPMW波，不过此时先禁止，等完成同步时再开始计数
    ...
    EPwm1Regs.ETSEL.bit.SOCASEL = ET_CTR_ZERO;      // ET事件选择(何时触发ET)
    EPwm1Regs.ETPS.bit.SOCAPRD = ET_1ST;            // 每一次事件都产生一个采样脉冲
    EPwm1Regs.ETSEL.bit.SOCBEN = 0;                 // 是否使能ADC启动脉冲SOC/B
    EPwm1Regs.ETSEL.bit.SOCBSEL = ET_CTR_ZERO;      // ET事件选择(何时触发ET)
...
{
    // 初始化GPIO，使用GPIO147、GPIO148，注意：28377有两对管脚可以产生PWM，此处注意配置
    InitEPwm2Gpio();
    EALLOW;
    // 设置TB周期计数器的值
    // 对于增减计数模式来说：F(EPWM)=PLLSYSCLK/2/TBPRD
    EPwm2Regs.TBPRD = 1000;                         // 设定EPWM频率为100k=200*1000*1000/2/1000
    // 设置比较寄存器的值
    EPwm2Regs.CMPA.bit.CMPA = (int)floor(0.5*1000); // 设定A通道占空比
    EPwm2Regs.CMPB.bit.CMPB = (int)floor(0.5*1000); // 设定B通道占空比
    EPwm2Regs.TBPHS.bit.TBPHS = 0;                  // 将相位寄存器清零
    EPwm2Regs.TBCTR = 0;                            // 将时间基准计数器清零
    // 设置EPWM模块的计数方式
    EPwm2Regs.TBCTL.bit.CTRMODE = TB_COUNT_UPDOWN;  // 设定为增减计数模式，产生对称的EPMW波
    EPwm2Regs.TBCTL.bit.PHSEN = TB_DISABLE;         // 禁止相位控制
    EPwm2Regs.TBCTL.bit.HSPCLKDIV = 0x00;           // 不分频
    EPwm2Regs.TBCTL.bit.CLKDIV = 0x00;              // 不分频
    // 设置EPWM动作方式
    EPwm2Regs.AQCTLA.bit.CAU = AQ_SET;              // 置位
    EPwm2Regs.AQCTLA.bit.CAD = AQ_CLEAR;
    EPwm2Regs.AQCTLB.bit.CBU = AQ_CLEAR;            // 清零
